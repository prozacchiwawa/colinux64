...
} co_linux_message_power_t;

typedef struct {
	uint32_t tick_count;
} co_linux_message_idle_t;
...
# ifdef __x86_64__
typedef uint64_t vm_ptr_t;
# else
typedef uint32_t vm_ptr_t;
# endif
#endif
...
typedef struct {
  uint64_t co_core_end;
  uint64_t co_memory_size;
  void *co_initrd;
